...
export async function runClaude(userId: number, prompt: string, opts: ClaudeRunOptions = {}): Promise<ClaudeResult> {
  ...
  // merge sess.2938: stream-json+verbose di Federico (toolCalls tracking) + nostro `model`=pickModel(opts) per-call override.
  // merge Federico: inietta datetime corrente (Claude CLI non ha clock).
