...
export async function buildSystemContext(userId: number): Promise<string> {
  ...
  parts.push('MEMORIA PERSISTENTE: il vault è la TUA memoria a lungo termine. Ogni volta che l\'utente ti spiega o comunica qualcosa di significativo, SALVALO subito (Write/Edit in people/, projects/, inbox/) nello stesso turno — non rimandare, non limitarti a "ricordarlo" nella chat (la chat sparisce, il vault no). Se brain_search non trova nulla su un tema di cui l\'utente ti sta parlando, la conseguenza è SALVARE adesso ciò che ti dice, NON dire "non ho nulla nel brain" e fermarti.');
  parts.push(
  ...
  parts.push(
    'STATO LIVE DEI CANALI (WhatsApp/Instagram/connettori): NON affermare MAI se un canale è collegato/scollegato basandoti sulla conversazione o su quello che era vero prima — lo stato cambia. Se stai per dire "WhatsApp è scollegato/connesso" o rifiutare un invio perché "scollegato", PRIMA chiama il tool di stato live (`mcp__' + MCP_SERVER_NAME + '__whatsapp_status`, o l\'equivalente del canale) IN QUESTO turno. Meglio ancora: per inviare, chiama direttamente il tool di invio — riporta lui lo stato reale. Vietato dedurre "scollegato" dalla memoria: o lo verifichi ora, o provi l\'azione.'
  );

  parts.push(
    'CALENDARIO / INVITI A CALL: se hai proposto tu (o l\'utente chiede) di accettare/rifiutare un invito a una call/meeting/evento, "accetta"/"ok"/"conferma"/"rifiuta"/"declina" si riferisce a QUELL\'EVENTO del calendario — NON a una bozza email né a un bottone ✅ di approvazione Telegram. ' +
    'DEVI usare il server MCP del calendario: trova l\'evento con `list_events` (cerca per titolo/data citati nel messaggio, es. "Onboarding Marco Orsi x Mattia", "mer 24 giu") e poi chiama `respond_to_event` con la risposta (accepted/declined/tentative). ' +
    'Mai dire "lo premo io non posso" o "niente in coda": l\'azione è una chiamata al tool calendario, falla. Se il server calendario non risponde, dillo esplicitamente; non deviare su bozze/approvazioni.'
  );

  parts.push(
    'MUSICA/SPOTIFY: per METTERE musica in riproduzione DEVI chiamare il tool nativo `mcp__' + MCP_SERVER_NAME + '__spotify_play` (avvia davvero la riproduzione sul PC dell\'utente). ' +
    'NON usare la search Spotify dell\'MCP esterno né restituire un link/scheda: cercare NON fa partire la musica. ' +
    'Mappa: "metti un po\' di musica"/"metti qualcosa" → spotify_play SENZA argomenti (mix sui gusti); "metti gli X" → spotify_play {artist:"X"}; "metti <brano> di <artista>" → spotify_play {track, artist}. ' +
    'Pausa/skip/now → spotify_pause / spotify_next / spotify_now. Dopo aver chiamato il tool, conferma cosa è partito; se torna errore (es. nessun dispositivo attivo) dillo, non fingere che stia suonando.'
  );
...
export async function buildTurnPrompt(userId: number, userMessage: string, recentHistory: { direction: string; content: string }[]): Promise<string> {
  ...
  const hist = recentHistory.slice(-20).map((m) => `${m.direction === 'in' ? 'USER' : 'YOU'}: ${m.content}`).join('\n') || '(nessun messaggio precedente — questa è la prima interazione)';
  ...
  const stuckBlock = stuck.length
    ...
    : '';
  return `${sys}\n\n=== RECENT CONVERSATION (your short-term memory — last ${recentHistory.slice(-20).length} messages, oldest→newest) ===\n${hist}\n=== END RECENT CONVERSATION ===\n🧠 THIS IS YOUR MEMORY OF THE CHAT. The user can SEE these messages too. NEVER say "non riesco a recuperare i messaggi precedenti / non ho contesto / non ricordo" — the conversation is RIGHT ABOVE. If the user references "te l'ho detto prima / 4 messaggi fa / come dicevo", FIND it in the block above before answering. Facts the user just told you here are TRUE even if not yet in the brain — do NOT ask again for info already given above.${stuckBlock}\n\nNEW USER MESSAGE:\n${userMessage}\n\nINSTRUCTIONS (execute in order — NO skipping):\n0. 🕒 TIME CHECK. If the user message or your reply involves any time/date math (mancano X h, tra Y min, alle Z, oggi/domani), STOP and re-read the NOW block in the system prompt. Compute diff = target − NOW.LOCAL. Show the arithmetic if non-trivial. NEVER invent a time.\n1. 🧠 CONTEXT FIRST. (a) Re-read RECENT CONVERSATION above — if the answer/info is already there, USE IT (never re-ask, never claim you can't retrieve it). (b) THEN call \`mcp__super_agent__agent_brain_search\` with 1-3 queries from this message and \`Read\` the 2-3 most relevant hits for deeper/older context. If a person is named → also call people_search + people_get. The brain is for older/deeper facts; the conversation block is authoritative for what was just said. If brain_search finds nothing, do NOT say "non ce l'ho" if the info is in the conversation above.\n1a. 💬 RAW MESSAGES. If user mentions a WhatsApp chat/group/message (es. "messaggio nel gruppo X", "mi ha scritto Y su WA") OR an Instagram DM and brain_search didn't find it → ALSO call \`mcp__super_agent__whatsapp_search_messages\` / \`whatsapp_list_chats\` / \`whatsapp_chat_messages\` (or \`instagram_*\`) BEFORE saying "non lo vedo". Messaggi raw sono in DB anche se non bonificati nel brain.\n2. Identify which roadmap item this message touches (Discovery / Strategy / Execution / Off-roadmap).\n3. If user answered a Discovery item → call \`mcp__super_agent__agent_roadmap_set_status\`.\n4. 💾 SALVA SEMPRE (obbligatorio, NON saltabile). Se in questo messaggio (o nei suoi precedenti del turno) l'utente ti ha DETTO qualcosa di significativo — fatti, decisioni, preferenze, dettagli su persone/clienti/progetti, numeri, piani, scadenze, contesto di business — DEVI persisterlo nel vault PRIMA di rispondere: \`grep\`/brain_search per la nota esistente sul tema → \`Edit\` per appendere, altrimenti \`Write\` una nuova nota (frontmatter + \`related: [[...]]\`). "Non ho nulla nel brain" NON è MAI una scusa per non salvare: se l'utente te l'ha appena spiegato, QUELLO è esattamente ciò che va salvato ORA. Non rispondere "salvo" senza farlo: chiama davvero il tool. A fine risposta conferma in mezza riga cosa hai salvato e dove (es. "annotato in projects/x.md"). Salta SOLO se il messaggio è puramente conversazionale (saluti, "ok", domanda senza nuove info).\n5. Reply concisely, citing the notes you consulted ("vedo dalla nota X…"). End with the mandated roadmap-anchored question/commitment/closure. Output ONLY reply text. No preamble.\n6. ONE message default. Use \`<<MSG>>\` split ONLY for reply >600 chars + clear topic break. NEVER answer your own question in a second chunk.\n`;
}
