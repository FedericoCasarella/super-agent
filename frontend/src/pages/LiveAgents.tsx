...
function StatusIcon({ status, size = 14 }: { status: string; size?: number }) {
  ...
  if (status === 'pending') return <ClockIcon {...common} className="text-muted-foreground" />;
  return <ClockIcon {...common} className="text-muted-foreground" />;
}
    ...
    <div className="space-y-5">
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <div className="border border-border rounded-2xl p-3 bg-surface2/40">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Token in</div>
            <div className="text-2xl font-semibold mt-1 font-mono">{(stats.totals?.in_tok ?? 0).toLocaleString()}</div>
          </div>
          <div className="border border-border rounded-2xl p-3 bg-surface2/40">
            <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Token out</div>
              ...
              <div key={p.id} className="border border-border rounded-2xl p-4 bg-surface2/40">
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div>
                    ...
                    {p.reason && <div className="text-sm text-muted-foreground mt-1">{p.reason}</div>}
                  </div>
                ...
                <ul className="text-sm space-y-1 mt-2 pl-3 border-l border-border">
                  {p.proposals.map((s, i) => (
                    <li key={i}><span className="font-medium">{s.title}</span> — <span className="text-muted-foreground">{s.brief}</span></li>
                  ))}
                </ul>
              </div>
            ))}
          ...
          <div className="max-w-3xl w-full max-h-[85vh] overflow-y-auto" onClick={(e: any) => e.stopPropagation()}><Card>
            <div className="flex items-start justify-between mb-3 gap-3 sticky top-0 bg-surface/95 backdrop-blur pb-2 -mx-4 px-4 z-10">
              <div className="min-w-0">
                ...
                {open.brief && <div className="text-sm text-muted-foreground mt-1 line-clamp-2">{open.brief}</div>}
              </div>
            ...
            <div className="grid grid-cols-3 md:grid-cols-6 gap-2 text-xs mb-4">
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Durata</div><div className="font-mono mt-1">{fmtDur(open.started_at, open.ended_at)}</div></div>
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Costo</div><div className="font-mono mt-1">{open.cost_usd != null ? `$${Number(open.cost_usd).toFixed(4)}` : '—'}</div></div>
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Token in</div><div className="font-mono mt-1">{open.input_tokens?.toLocaleString() ?? '—'}</div></div>
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Token out</div><div className="font-mono mt-1">{open.output_tokens?.toLocaleString() ?? '—'}</div></div>
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Turni</div><div className="font-mono mt-1">{open.num_turns ?? '—'}</div></div>
              <div className="border border-border rounded-xl p-2"><div className="text-muted-foreground">Creato</div><div className="font-mono mt-1">{fmtAgo(open.created_at)}</div></div>
            </div>

            {open.actions?.length > 0 && (
              <div className="mb-4">
                <div className="text-xs text-muted-foreground mb-2 font-semibold uppercase">Azioni eseguite ({open.actions.length})</div>
                  ...
                  {open.actions.map((a, i) => (
                    <li key={i} className="flex items-start gap-2 font-mono min-w-0">
                      ...
                      <div className="flex-1 min-w-0 overflow-hidden">
                        ...
                        {a.brief && (
                          <span className="block text-muted-foreground whitespace-pre-wrap break-all mt-0.5">{a.brief}</span>
                        )}
                      </div>
                    </li>
                  ))}
            ...
            <details className="mb-3"><summary className="text-xs text-muted-foreground cursor-pointer">Prompt completo</summary><pre className="text-xs bg-surface2/40 border border-border rounded-xl p-3 mt-2 whitespace-pre-wrap overflow-auto max-h-64">{open.prompt}</pre></details>
            {open.error && (
